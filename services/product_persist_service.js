const {Brand, Product, Source, Image} = require('./../models');
...
const fetchProductDetails = (payload, brand) => {
    ...
    productDetails['item_id'] = payload.sephoraProductId;
...
class ProductPersistService extends BaseService {
    ...
    async checkProductExists() {
        ...
        const [product, errors] = await syncError(Product.findOne({ where: { name: productDetails.name,
                                   item_id: productDetails.item_id,
                                    upc: productDetails.upc}}))

        if(product){
            this.addErrors(`Product with id: ${product.id} already exists.`);
            this.errorCode = 'product_exists';
            return;
        }
    }
    ...
    async createProduct() {
        ...
        const productDetails = fetchProductDetails(this.productPayload, this.brand);
        const [product, errors] = await syncError(Product.create({...productDetails}));
    ...
    async createImages() {
        ...
        const [images, errors] = await syncError(Image.bulkCreate(imageDetails));
        debugger;
        if (any(errors)){
            this.errorCode = 'image_creation_error'
            this.addErrors(errors);
        } else {
            this.images = images;
        }
