...
const fetchLabels = async (service) => {
    try {
        ...
        logger.info({ src: 'Coordinator', event: 'fetchLabels', status: 'completed', data: { labels: labelsDetected, imageBlob: imageBlob } });
    } catch (e) {
        logger.info({ src: 'Coordinator', event: 'fetchLabels', status: 'completed', data: { labels: labelsDetected, imageBlob: imageBlob } });
...
const persistLabels = async (service) => {
    ...
    if (any(errors)) {
        ...
        logger.error({ src: 'Coordinator', event: 'persistLabels', data: { imageBlob: service.imageBlob, detectedLabels: service.detectedLabels }, errors: { message: errors, stack: null } })
    }
